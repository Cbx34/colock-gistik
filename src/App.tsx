...
export default function App() {
  ...
  const notConnected = () => setActionMessage('Fonction non encore connectée');
  const runApify = async () => {
    ...
    try {
      ...
      addApifyMessage(`Erreur : connexion Apify impossible. ${message}`);
      const fallback = mockProspectSearch(criteria);
      const merged = mergeProspects(prospects, fallback);
      setProspects(merged.prospects);
      addApifyMessage(`Données test visibles remises : ${merged.added} ajoutées, ${merged.merged} doublons.`);
    } finally {
  ...
  return <Layout activePage={activePage in pages ? activePage : 'dashboard'} title={current.title} subtitle={current.subtitle} supabaseStatus={connection.connected ? 'connected' : 'disconnected'}>{actionMessage ? <p className="notice">{actionMessage}</p> : null}{connection.error ? <p className="notice danger-notice">Erreur : {connection.error}</p> : null}{connection.initializing ? <p className="notice">Connexion à Supabase en cours…</p> : null}{current.element}</Layout>;
}
