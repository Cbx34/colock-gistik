...
export default function App() {
  ...
  const runAutoProspecting = async () => {
    setApifyMessages([`Prospection automatique lancée sur ${ECOMMERCE_KEYWORD_QUERIES.length} mots-clés e-commerce.`]);
    setAutoProspecting(true);
    setApifyLoading(true);
    try {
      for (const item of ECOMMERCE_KEYWORD_QUERIES) {
        addApifyMessage(`▶ ${item.category} — ${item.keyword}`);
        const autoCriteria: SearchCriteria = { platform: 'Shopify', productType: item.keyword, location: 'France', keywords: item.query };
        const result = await fetchApifyProspects(apifyActor, apifyToken, autoCriteria, undefined, (progress: ApifyProgress) => addApifyMessage(`${item.keyword} : ${progress.message}`));
        setShopifyRawResults(result.rawItems ?? []);
        ...
        addApifyMessage(`✓ ${item.keyword} : ${result.insertedCount ?? result.prospects.length} insérés, ${result.duplicateCount ?? 0} doublons.`);
      }
      addApifyMessage('Prospection automatique terminée : Supabase est alimenté en continu pendant toute la séquence.');
    } catch (error) {
      const message = error instanceof Error ? error.message : 'Erreur Apify inconnue';
