...
export default function App() {
  ...
  const pages: Record<PageKey, { title: string; subtitle: string; element: ReactNode }> = {
    dashboard: { title: 'Prospection automatique Colock-Gistik', subtitle: 'MVP simple pour trouver, scorer, contacter et relancer des vendeurs e-commerce.', element: <div className="page-grid"><section className="orange-hero"><div><span className="pill">Stockage · Préparation · Expédition</span><h2>Pipeline commercial e-commerce</h2><p>Centralisez les boutiques Shopify, Vinted Pro, TikTok Shop, Etsy, eBay, dropshippers et petites marques françaises à prospecter.</p></div><div className="module-actions"><a className="big-action" href="#search"><Plus/> Lancer une recherche</a><button className="big-action" onClick={enrichProspects} disabled={apifyLoading}><RefreshCw/> Enrichir les prospects</button></div></section><div className="stats-grid"><Stat label="prospects trouvés" value={metrics.found}/><Stat label="prospects qualifiés" value={metrics.qualified}/><Stat label="emails trouvés" value={metrics.emails}/><Stat label="téléphones trouvés" value={metrics.phones}/><Stat label="formulaires de contact" value={metrics.forms}/><Stat label="Instagram" value={metrics.instagram}/><Stat label="Facebook" value={metrics.facebook}/><Stat label="TikTok" value={metrics.tiktok}/><Stat label="mono-produits" value={metrics.monoProducts}/><Stat label="petites boutiques" value={metrics.smallStores}/><Stat label="taux de qualification" value={`${metrics.qualificationRate}%`}/></div><section className="panel wide-panel"><h3>Priorité du jour</h3>{prospects.filter((p)=>(p.classement==='ultra-chaud' || p.classement==='chaud') && p.statutContact!=='Supprimé').slice(0,4).map((p)=><ProspectCard key={p.id} p={p} onSelect={()=>select(p.id)} onContact={()=>markContacted(p.id)} onDelete={()=>deleteProspect(p.id)}/>)}</section><aside className="panel"><h3><ShieldCheck/> RGPD</h3><p>Données publiques uniquement, pas de spam massif, historique de contact et suppression immédiate via le bouton Supprimer.</p></aside></div> },
