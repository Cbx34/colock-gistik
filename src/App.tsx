...
import { Download, ExternalLink, Mail, Plus, RefreshCw, Search as SearchIcon, ShieldCheck, Trash2, Upload, Rocket, ShoppingBag, BarChart3 } from 'lucide-react';
import Layout, { type PageKey } from './components/Layout';
import { buildSearchLinks, defaultCampaigns, fetchApifyProspects, followUpDays, generateMessage, mergeProspects, mockProspectSearch, parseCsvProspects, prospectStatuses, scoreProspect, type Campaign, type Platform, type Prospect, type SearchCriteria } from './lib/prospecting';
...
function ProspectCard({ p, onSelect, onDelete, onContact }: { p: Prospect; onSelect: () => void; onDelete: () => void; onContact: () => void }) {
  return <article className="prospect-card"><div><h3>{p.nomBoutique}</h3><p>{p.typeProduits} · {p.ville}, {p.pays}</p><div className="chips"><Badge tone={p.classement}>{p.classement}</Badge><Badge>{p.plateforme}</Badge><Badge>Score potentiel {p.score}/10</Badge><Badge>{p.statutContact}</Badge></div></div><ul>{p.volumeSignaux.map((s) => <li key={s}>{s}</li>)}{p.siteWeb ? <li><a href={p.siteWeb} target="_blank">Site web</a></li> : null}{p.email ? <li>{p.email}</li> : null}{p.telephone ? <li>{p.telephone}</li> : null}{p.sourceUrl ? <li><a href={p.sourceUrl} target="_blank">URL source</a></li> : null}</ul><div className="card-actions"><button onClick={onSelect}>Ouvrir</button><button onClick={onContact}><Mail size={17}/> Contacté</button><button className="secondary danger" onClick={onDelete}><Trash2 size={17}/> Supprimer</button></div></article>;
}
...
export default function App() {
  ...
  const [apifyActor, setApifyActor] = useState(import.meta.env.VITE_APIFY_ACTOR_ID ?? 'compass/crawler-google-places');
  ...
  const runApify = async () => {
    setApifyMessage('');
    if (!apifyToken.trim()) { setApifyMessage('Erreur token Apify'); return; }
    setApifyLoading(true);
    setApifyMessage('Recherche Apify lancée');
    try {
      const result = await fetchApifyProspects(apifyActor, apifyToken, criteria);
      if (!result.prospects.length) { setApifyMessage('Aucun résultat'); return; }
      ...
      setApifyMessage(`${merged.added} prospects importés${merged.merged ? ` · ${merged.merged} doublons fusionnés` : ''}`);
    } catch (error) {
      const message = error instanceof Error ? error.message : 'Erreur Apify';
      setApifyMessage(/token|401|403/i.test(message) ? 'Erreur token Apify' : message);
    } finally {
  ...
  const pages: Record<PageKey, { title: string; subtitle: string; element: ReactNode }> = {
    ...
    search: { title: 'Recherche prospects', subtitle: 'Générez des pistes et ouvrez les recherches web/sociales publiques en un clic.', element: <div className="module-page"><div className="form-card"><h3><SearchIcon/> Critères</h3><div className="form-grid"><select value={criteria.platform} onChange={(e)=>setCriteria({...criteria, platform:e.target.value as Platform | 'Toutes'})}>{platforms.map((p)=><option key={p}>{p}</option>)}</select><input placeholder="Type produits" value={criteria.productType} onChange={(e)=>setCriteria({...criteria, productType:e.target.value})}/><input placeholder="Ville / pays" value={criteria.location} onChange={(e)=>setCriteria({...criteria, location:e.target.value})}/><input placeholder="Mots-clés" value={criteria.keywords} onChange={(e)=>setCriteria({...criteria, keywords:e.target.value})}/></div><div className="module-actions"><button onClick={runApify} disabled={apifyLoading}><SearchIcon/> {apifyLoading ? 'Recherche en cours…' : 'Chercher'}</button><button onClick={addProspects} className="secondary"><Plus/> Ajouter prospects démo</button></div><p className="form-message">Synchronisation : {connection.connected ? 'Supabase connecté' : 'Supabase non connecté'}</p>{apifyMessage ? <p className="form-message apify-status">{apifyMessage}</p> : null}</div><div className="form-card"><h3><Upload/> Imports</h3><textarea value={importText} onChange={(e)=>setImportText(e.target.value)} /><div className="module-actions"><button onClick={()=>importCsv('CSV')}><Upload/> Import CSV</button><button onClick={()=>importCsv('Shopify')}><ShoppingBag/> Import Shopify</button><button onClick={()=>importCsv('TikTok Shop')}><Rocket/> Import TikTok Shop</button></div><div className="form-grid"><input placeholder="Apify Actor ID" value={apifyActor} onChange={(e)=>setApifyActor(e.target.value)}/><input placeholder="Token Apify (masqué)" type="password" autoComplete="off" value={apifyToken} onChange={(e)=>setApifyToken(e.target.value)}/><button onClick={runApify} disabled={apifyLoading}><Rocket/> {apifyLoading ? 'Connexion…' : 'Connecter Apify'}</button></div><p className="form-message">Le token Apify reste masqué et n’est jamais affiché en clair.</p>{apifyMessage ? <p className="form-message apify-status">{apifyMessage}</p> : null}</div><div className="card-grid">{buildSearchLinks(criteria).map((l)=><a className="work-card link-card" href={l.url} target="_blank" key={l.label}><ExternalLink/><strong>{l.label}</strong><span>Source publique à vérifier avant import.</span></a>)}</div></div> },
    prospects: { title: 'Liste prospects', subtitle: 'Classement chaud / moyen / faible selon signaux de volume, contacts publics et proximité.', element: <div className="module-page"><div className="module-actions"><button onClick={()=>exportProspectsCsv(prospects)}><Download/> Export CSV</button></div><div className="prospect-list">{prospects.filter((p)=>p.statutContact!=='Supprimé').map((p)=><ProspectCard key={p.id} p={p} onSelect={()=>select(p.id)} onContact={()=>markContacted(p.id)} onDelete={()=>deleteProspect(p.id)}/>)}</div></div> },
    prospect: { title: 'Fiche prospect', subtitle: 'Informations utiles, score, message personnalisé et suppression RGPD.', element: <div className="module-page">{selected ? <><ProspectCard p={selected} onSelect={()=>{}} onContact={()=>markContacted(selected.id)} onDelete={()=>deleteProspect(selected.id)}/><div className="form-card"><h3>Modifier</h3><div className="form-grid"><input value={selected.email ?? ''} placeholder="Email public" onChange={(e)=>updateSelected({email:e.target.value})}/><input value={selected.telephone ?? ''} placeholder="Téléphone public" onChange={(e)=>updateSelected({telephone:e.target.value})}/><input value={selected.siteWeb ?? ''} placeholder="Site web" onChange={(e)=>updateSelected({siteWeb:e.target.value})}/><select value={selected.statutContact} onChange={(e)=>updateSelected({statutContact:e.target.value as Prospect['statutContact']})}>{prospectStatuses.map((status)=><option key={status}>{status}</option>)}</select><textarea value={selected.notes ?? ''} onChange={(e)=>updateSelected({notes:e.target.value})}/></div></div><pre className="message-box">{generateMessage(selected)}</pre></> : <p>Aucun prospect.</p>}</div> },
