      ...
      for (const country of AUTO_PROSPECTING_COUNTRIES) {
        ...
        for (const group of ECOMMERCE_KEYWORD_LIBRARY) {
          ...
          for (const keyword of group.keywords) {
            ...
            if (result.report) addApifyMessage(`Rapport ${keyword} : ${result.report.rawCount} bruts · ${result.report.normalizedCount} normalisés · ${result.report.insertedCount} insérés · ${result.report.duplicateCount} doublons · ${result.report.rejectedCount} rejetés`);
            if (connection.connected) {
