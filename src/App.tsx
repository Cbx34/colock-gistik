...
export default function App() {
  ...
  const dashboardKpis = [
    ['CA du jour', '2 840 €', '+12%', BarChart3], ['Colis reçus', '126', '+18', PackageCheck], ['Colis expédiés', '98', '96% SLA', Truck], ['Préparations en attente', '14', 'à traiter', Warehouse],
    ['Box libres', '37', 'sur 120', PackageCheck], ['Impayés', '6 420 €', '8 factures', Calculator], ['Alertes', '5', 'priorité haute', AlertTriangle], ['Nouveaux prospects', metrics.found, `${metrics.qualificationRate}% qualifiés`, Bot],
  ] as const;
  const dashboardElement = <div className="saas-page">
    <section className="premium-hero"><div><span className="pill">COLOCK-BOX · Pilotage temps réel</span><h2>Une console claire pour piloter stockage, préparation, expédition et croissance.</h2><p>Vue opérationnelle quotidienne, indicateurs prioritaires, prospection IA et actions rapides dans une interface responsive.</p></div><div className="module-actions"><a className="big-action" href="#logistique"><Warehouse/> Voir la logistique</a><a className="big-action light" href="#prospection"><Bot/> Assistant IA</a></div></section>
    <div className="kpi-grid">{dashboardKpis.map(([label,value,trend,Icon])=><article className="kpi-card" key={label}><div><span>{label}</span><strong>{value}</strong><small>{trend}</small></div><Icon size={24}/></article>)}</div>
    <section className="analytics-grid"><article className="panel chart-panel"><div className="panel-title"><LineChart/><h3>Graphiques</h3></div><div className="bars"><i style={{height:'58%'}}/><i style={{height:'82%'}}/><i style={{height:'46%'}}/><i style={{height:'72%'}}/><i style={{height:'64%'}}/><i style={{height:'90%'}}/><i style={{height:'76%'}}/></div><p className="notice inline-notice">Volumes hebdomadaires simulés côté interface, sans modifier les données métier.</p></article><article className="panel"><div className="panel-title"><Users/><h3>Top clients</h3></div><div className="data-list compact">{hotProspects.slice(0,5).map((p)=><button className="client-line" onClick={()=>select(p.id)} key={p.id}><strong>{p.nomBoutique}</strong><span>{p.ville} · score {p.score}/100</span></button>)}{!hotProspects.length ? <p className="notice inline-notice">Aucun client prioritaire pour le moment.</p> : null}</div></article></section>
    <section className="panel"><div className="panel-title"><Warehouse/><h3>Préparations en cours</h3></div><div className="data-table"><div><strong>Commande</strong><strong>Client</strong><strong>Statut</strong><strong>Priorité</strong></div>{activeProspects.slice(0,6).map((p,i)=><div key={p.id}><span>PREP-{String(i+1248).padStart(4,'0')}</span><span>{p.nomBoutique}</span><span>Picking & packing</span><em>{p.classement}</em></div>)}</div></section>
  </div>;
