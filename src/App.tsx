...
export default function App() {
  ...
  const [filters, setFilters] = useState({ emailOnly: false, phoneOnly: false, franceOnly: false, monoOnly: false, shopifyOnly: false, score80: false, score90: false });
  ...
  const visibleProspects = prospects.filter((p) => p.statutContact !== 'Supprimé'
    ...
    && (!filters.score80 || p.score > 80)
    && (!filters.score90 || p.score > 90));
      ...
      for (const country of AUTO_PROSPECTING_COUNTRIES) {
        ...
        for (const group of ECOMMERCE_KEYWORD_LIBRARY) {
          ...
          for (const keyword of group.keywords) {
            ...
            const result = await fetchApifyProspects(apifyActor, apifyToken, autoCriteria, batchSize, (progress: ApifyProgress) => addApifyMessage(`${country}/${keyword} : ${progress.message}`));
            setShopifyRawResults(result.rawItems ?? []);
  ...
  const pages: Record<PageKey, { title: string; subtitle: string; element: ReactNode }> = {
    ...
    prospects: { title: 'Liste prospects', subtitle: 'Classement 🔥 Ultra chaud / 🟢 Chaud / 🟡 Moyen / ⚪ Faible selon signaux de volume, contacts publics et proximité.', element: <div className="module-page"><div className="module-actions"><button onClick={()=>exportProspectsCsv(prospects)}><Download/> Export CSV</button>{[
      ['emailOnly','Email uniquement'], ['phoneOnly','Téléphone uniquement'], ['franceOnly','France uniquement'], ['monoOnly','Mono-produit uniquement'], ['shopifyOnly','Shopify vérifié uniquement'], ['score80','Score >80'], ['score90','Score >90']
    ...
    campaigns: { title: 'Campagnes', subtitle: 'Suivez les campagnes de prospection, leurs sources et leurs statistiques.', element: <div className="module-page"><div className="stats-grid"><Stat label="prospects trouvés" value={metrics.found}/><Stat label="prospects qualifiés" value={metrics.qualified}/><Stat label="emails disponibles" value={metrics.emails}/><Stat label="ultra chauds" value={`${metrics.ultraRate}%`}/></div><div className="prospect-list">{campaigns.map((c)=><article className="panel campaign-card" key={c.id}><h3><BarChart3/> {c.nom}</h3><p>{c.cible}</p><div className="chips"><Badge>{c.statut}</Badge><Badge>Apify · Shopify · TikTok Shop · CSV</Badge></div></article>)}</div></div> },
